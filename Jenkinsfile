pipeline{
    ...
    stages{
        stage('Build maven'){
            steps{
                checkout([$class: 'GitSCM', branches: [[name: '*/master']], extensions: [], userRemoteConfigs: [[credentialsId: 'd8a7d834-0955-4bfe-a691-25bbca4ab26b', url: 'https://nextgentechsavvy@github.com/nextgentechsavvy/mmtspl-cloud-config-server.git']]])
        ...
        stage('Build docker image'){
            steps{
                         ...
                         bat 'docker build -t mmtspl-cloud-config-server-1.0.0-snapshot .'
            }
        }
        ...
        stage('Tag docker image to the DockerHub image'){
            steps{
                    ...
                    bat 'docker image tag mmtspl-cloud-config-server-1.0.0-snapshot:latest nextgentechsavvy/mmtspl-cloud-config-server-1.0.0-snapshot:latest-hub-image'
            }
        }
        stage('Push docker hub image to the DockerHub'){
            steps{
                    ...
                    //bat 'echo "Temporarily skipped this steps..."'
                    bat 'docker push nextgentechsavvy/mmtspl-cloud-config-server-1.0.0-snapshot:latest-hub-image'
            }
        }
        stage('Deploy to k8s'){
            steps{
